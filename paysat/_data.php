...
$sql = "SELECT * FROM data_plan WHERE id='$plan_id'";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        //price	network	data_size 	
...
<form name="prompt-cash-form" action="https://prompt.cash/pay" method="get">
    ...
    <input type="hidden" name="return" value="http://localhost/fay/paysat/success.php?id=<?php echo $id?>">
...
<?php 
//Full texts 	id	phone	amount	network	network_title	status	t_type	data_size 	
new_order($id,$phone,$ngn,$network_code,$network_title,$data_size,$servername, $username, $password, $dbname);
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
//new_order($id,$phone,$amount,$network_code,$network_title,$servername, $username, $password, $dbname);
function new_order($id,$phone,$amount,$network,$network_title,$data_size,$servername, $username, $password, $dbname){
    ...
    $sql = "INSERT INTO airtime_payment (id,phone,amount,network,network_title,status,t_type,data_size)
VALUES ('$id', '$phone', '$amount','$network', '$network_title', 'Pending','Data','$data_size')";
